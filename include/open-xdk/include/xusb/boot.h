...
#define ASSERT(exp)                                                         \
    {                                                                       \
        if(!(exp))                                                          \
        {                                                                   \
            bprintf("Assert failed file " __FILE__ " line %d\n", __LINE__); \
        }                                                                   \
    }
...
typedef struct
{ // inside an 8-byte protected mode interrupt vector
...
typedef struct _AUDIO_ELEMENT
{ // represents a kind of generated sound, needs a derived object to be usable
...
typedef struct _AUDIO_ELEMENT_SINE
{ // derived object composed from base object AUDIO_ELEMENT represents a sine wave with harmonics
...
typedef struct _AUDIO_ELEMENT_NOISE
{ // derived object composed from base object AUDIO_ELEMENT represents a whitish noise source
    AUDIO_ELEMENT m_paudioelement;
...
typedef struct
{ // descriptor from AC97 specification
