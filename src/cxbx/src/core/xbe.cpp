...
// * BetterTime
// ******************************************************************
static const char* BetterTime(uint32 timestamp)
{
    // XBE timestamps are 32-bit; the host CRT's time_t can be 64-bit.
    const time_t host_time = static_cast<time_t>(timestamp);
    char* text = ctime(&host_time);
    if(text == nullptr)
    {
