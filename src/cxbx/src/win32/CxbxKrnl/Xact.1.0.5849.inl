// XACT 5849 signatures generated from xacteng.lib.

// _XACTEngineDoWork@0 (xacteng.lib, 50 bytes)
SOOVPA<13> XACTEngineDoWork_1_0_5849 =
{
    0, 13, -1, 0,
    {
        { 0x00, 0x56 }, { 0x02, 0xE8 }, { 0x08, 0x35 },
        { 0x0D, 0x85 }, { 0x10, 0xB6 }, { 0x14, 0xE8 },
        { 0x19, 0x8B }, { 0x1B, 0xE8 }, { 0x20, 0x85 },
        { 0x24, 0x68 }, { 0x29, 0xFF }, { 0x2A, 0x15 },
        { 0x31, 0xC3 }
    }
};

// _XACTEngineCreate@8 (xacteng.lib, 165 bytes)
SOOVPA<13> XACTEngineCreate_1_0_5849 =
{
    0, 13, -1, 0,
    {
        { 0x00, 0x53 }, { 0x0C, 0xA1 }, { 0x1B, 0x08 },
        { 0x29, 0xEB }, { 0x36, 0xE8 }, { 0x46, 0x8B },
        { 0x52, 0x81 }, { 0x5F, 0x24 }, { 0x6D, 0x85 },
        { 0x7B, 0xD8 }, { 0x88, 0x57 }, { 0x98, 0xFF },
        { 0xA4, 0x00 }
    }
};

// _IXACTEngine_AddRef@4 (xacteng.lib, 55 bytes)
SOOVPA<13> IXACTEngine_AddRef_1_0_5849 =
{
    0, 13, -1, 0,
    {
        { 0x00, 0x56 }, { 0x02, 0xE8 }, { 0x09, 0xF0 },
        { 0x0D, 0x0C }, { 0x12, 0xF8 }, { 0x16, 0xC9 },
        { 0x1A, 0xE8 }, { 0x1F, 0x85 }, { 0x24, 0x0B },
        { 0x2A, 0xFF }, { 0x2B, 0x15 }, { 0x31, 0xC7 },
        { 0x36, 0x00 }
    }
};

// ?Release@CEngine@XACT@@QAGKXZ (xacteng.lib, 47 bytes)
SOOVPA<13> XACTEngine_ReleaseInternal_1_0_5849 =
{
    0, 13, XREF_XACT5849_ENGINE_RELEASE, 0,
    {
        { 0x00, 0x56 }, { 0x02, 0xBE }, { 0x07, 0x56 },
        { 0x09, 0x15 }, { 0x0F, 0x4C }, { 0x13, 0x49 },
        { 0x17, 0x08 }, { 0x1A, 0x6A }, { 0x1C, 0xE8 },
        { 0x22, 0xFF }, { 0x28, 0x8B }, { 0x2A, 0x5F },
        { 0x2E, 0x00 }
    }
};

// _IXACTEngine_Release@4 (call@0x11 -> XREF_XACT5849_ENGINE_RELEASE)
SOOVPA<14> IXACTEngine_Release_1_0_5849 =
{
    0, 14, -1, 1,
    {
        { 0x11, XREF_XACT5849_ENGINE_RELEASE },
        { 0x00, 0x8B }, { 0x01, 0x44 }, { 0x02, 0x24 },
        { 0x03, 0x04 }, { 0x05, 0xC8 }, { 0x07, 0xC0 },
        { 0x09, 0xF7 }, { 0x0B, 0x1B }, { 0x0D, 0x23 },
        { 0x0F, 0x51 }, { 0x10, 0xE8 }, { 0x15, 0xC2 },
        { 0x17, 0x00 }
    }
};

// ?RegisterWaveBank@CEngine@XACT@@QAGJPAXKPAPAUIXACTWaveBank@@@Z
SOOVPA<13> XACTEngine_RegisterWaveBankInternal_1_0_5849 =
{
    0, 13, XREF_XACT5849_REGISTER_WAVEBANK, 0,
    {
        { 0x00, 0x53 }, { 0x0A, 0x84 }, { 0x15, 0xE8 },
        { 0x1F, 0xC8 }, { 0x2A, 0xDF }, { 0x34, 0xC7 },
        { 0x3F, 0x8B }, { 0x4A, 0x8B }, { 0x54, 0x8D },
        { 0x5E, 0x04 }, { 0x69, 0x89 }, { 0x72, 0xE8 },
        { 0x7E, 0x00 }
    }
};

// _IXACTEngine_RegisterWaveBank@16
// call@0x27 -> XREF_XACT5849_REGISTER_WAVEBANK
SOOVPA<14> IXACTEngine_RegisterWaveBank_1_0_5849 =
{
    0, 14, -1, 1,
    {
        { 0x27, XREF_XACT5849_REGISTER_WAVEBANK },
        { 0x00, 0x56 }, { 0x07, 0xFF }, { 0x0B, 0x0F },
        { 0x10, 0x24 }, { 0x16, 0x8B }, { 0x1B, 0x18 },
        { 0x21, 0x1B }, { 0x26, 0xE8 }, { 0x2C, 0xF6 },
        { 0x31, 0x68 }, { 0x37, 0x15 }, { 0x3C, 0x8B },
        { 0x42, 0x00 }
    }
};

// ?CreateSoundBank@CEngine@XACT@@QAGJPAXKPAPAUIXACTSoundBank@@@Z
SOOVPA<13> XACTEngine_CreateSoundBankInternal_1_0_5849 =
{
    0, 13, XREF_XACT5849_CREATE_SOUNDBANK, 0,
    {
        { 0x00, 0x56 }, { 0x09, 0xE8 }, { 0x14, 0xE8 },
        { 0x1E, 0xF6 }, { 0x28, 0xFF }, { 0x32, 0x36 },
        { 0x3C, 0x14 }, { 0x46, 0x7C }, { 0x50, 0xC1 },
        { 0x5A, 0x89 }, { 0x64, 0x18 }, { 0x6E, 0xE8 },
        { 0x79, 0x00 }
    }
};

// _IXACTEngine_CreateSoundBank@16
// call@0x27 -> XREF_XACT5849_CREATE_SOUNDBANK
SOOVPA<14> IXACTEngine_CreateSoundBank_1_0_5849 =
{
    0, 14, -1, 1,
    {
        { 0x27, XREF_XACT5849_CREATE_SOUNDBANK },
        { 0x00, 0x56 }, { 0x07, 0xFF }, { 0x0B, 0x0F },
        { 0x10, 0x24 }, { 0x16, 0x8B }, { 0x1B, 0x18 },
        { 0x21, 0x1B }, { 0x26, 0xE8 }, { 0x2C, 0xF6 },
        { 0x31, 0x68 }, { 0x37, 0x15 }, { 0x3C, 0x8B },
        { 0x42, 0x00 }
    }
};

// _IXACTSoundBank_GetSoundCueIndexFromFriendlyName@12
SOOVPA<13> IXACTSoundBank_GetSoundCueIndexFromFriendlyName_1_0_5849 =
{
    0, 13, -1, 0,
    {
        { 0x00, 0x56 }, { 0x02, 0xE8 }, { 0x08, 0x74 },
        { 0x0C, 0xB6 }, { 0x10, 0x24 }, { 0x14, 0x24 },
        { 0x1B, 0x85 }, { 0x1D, 0x8B }, { 0x21, 0x68 },
        { 0x26, 0xFF }, { 0x27, 0x15 }, { 0x2D, 0xC7 },
        { 0x32, 0x00 }
    }
};

// ?AddRef@CSoundBank@XACT@@QAGKXZ
SOOVPA<8> XACTSoundBank_AddRefInternal_1_0_5849 =
{
    0, 8, XREF_XACT5849_SOUNDBANK_ADDREF, 0,
    {
        { 0x00, 0x8B }, { 0x01, 0x44 }, { 0x02, 0x24 },
        { 0x04, 0xFF }, { 0x05, 0x00 }, { 0x07, 0x00 },
        { 0x08, 0xC2 }, { 0x0A, 0x00 }
    }
};

// ??1CSoundBank@XACT@@QAE@XZ
SOOVPA<13> XACTSoundBank_DestructorInternal_1_0_5849 =
{
    0, 13, XREF_XACT5849_SOUNDBANK_DESTRUCTOR, 0,
    {
        { 0x00, 0x53 }, { 0x0F, 0x3F }, { 0x1F, 0x0D },
        { 0x2F, 0x8B }, { 0x3F, 0x8D }, { 0x4F, 0x89 },
        { 0x5F, 0x9C }, { 0x6E, 0x24 }, { 0x7D, 0x35 },
        { 0x8E, 0xE8 }, { 0x9E, 0xE8 }, { 0xAE, 0x64 },
        { 0xBA, 0xE9 }
    }
};

// ??_GCSoundBank@XACT@@QAEPAXI@Z
// call@0x04 -> XREF_XACT5849_SOUNDBANK_DESTRUCTOR
SOOVPA<9> XACTSoundBank_DeletingDestructor_1_0_5849 =
{
    0, 9, XREF_XACT5849_SOUNDBANK_DELETING_DESTRUCTOR, 1,
    {
        { 0x04, XREF_XACT5849_SOUNDBANK_DESTRUCTOR },
        { 0x00, 0x56 }, { 0x01, 0x8B }, { 0x02, 0xF1 },
        { 0x03, 0xE8 }, { 0x08, 0xF6 }, { 0x0D, 0x74 },
        { 0x15, 0xE8 }, { 0x1F, 0x00 }
    }
};

// ?Release@CSoundBank@XACT@@QAGKXZ
// call@0x0E -> XREF_XACT5849_SOUNDBANK_DELETING_DESTRUCTOR
SOOVPA<9> XACTSoundBank_ReleaseInternal_1_0_5849 =
{
    0, 9, XREF_XACT5849_SOUNDBANK_RELEASE, 1,
    {
        { 0x0E, XREF_XACT5849_SOUNDBANK_DELETING_DESTRUCTOR },
        { 0x00, 0x8B }, { 0x02, 0x24 }, { 0x04, 0xFF },
        { 0x07, 0x8B }, { 0x09, 0x75 }, { 0x0D, 0xE8 },
        { 0x12, 0x8B }, { 0x17, 0x00 }
    }
};

// _IXACTSoundBank_AddRef@4
// call@0x0F -> XREF_XACT5849_SOUNDBANK_ADDREF
SOOVPA<14> IXACTSoundBank_AddRef_1_0_5849 =
{
    0, 14, -1, 1,
    {
        { 0x0F, XREF_XACT5849_SOUNDBANK_ADDREF },
        { 0x00, 0x56 }, { 0x01, 0x57 }, { 0x02, 0xE8 },
        { 0x07, 0xFF }, { 0x0B, 0x0F }, { 0x0E, 0xE8 },
        { 0x13, 0x85 }, { 0x18, 0x0B }, { 0x1E, 0xFF },
        { 0x1F, 0x15 }, { 0x24, 0x8B }, { 0x28, 0xC2 },
        { 0x2A, 0x00 }
    }
};

// _IXACTSoundBank_Release@4
// call@0x0F -> XREF_XACT5849_SOUNDBANK_RELEASE
SOOVPA<14> IXACTSoundBank_Release_1_0_5849 =
{
    0, 14, -1, 1,
    {
        { 0x0F, XREF_XACT5849_SOUNDBANK_RELEASE },
        { 0x00, 0x56 }, { 0x01, 0x57 }, { 0x02, 0xE8 },
        { 0x07, 0xFF }, { 0x0B, 0x0F }, { 0x0E, 0xE8 },
        { 0x13, 0x85 }, { 0x18, 0x0B }, { 0x1E, 0xFF },
        { 0x1F, 0x15 }, { 0x24, 0x8B }, { 0x28, 0xC2 },
        { 0x2A, 0x00 }
    }
};

// _IXACTEngine_UnRegisterWaveBank@8 (xacteng.lib, 59 bytes)
SOOVPA<13> IXACTEngine_UnRegisterWaveBank_1_0_5849 =
{
    0, 13, -1, 0,
    {
        { 0x00, 0x56 }, { 0x02, 0xE8 }, { 0x09, 0x24 },
        { 0x0E, 0x8B }, { 0x13, 0xC8 }, { 0x18, 0xD9 },
        { 0x1D, 0x51 }, { 0x23, 0x85 }, { 0x26, 0xF8 },
        { 0x29, 0x68 }, { 0x2F, 0x15 }, { 0x35, 0xC7 },
        { 0x3A, 0x00 }
    }
};

// ?Prepare@CSoundBank@XACT@@QAGJPBU_XACT_PREPARE_SOUNDCUE@@PAPAUIXACTSoundCue@@@Z
SOOVPA<8> XACTSoundBank_PrepareInternal_1_0_5849 =
{
    0, 8, XREF_XACT5849_SOUNDBANK_PREPARE, 0,
    {
        { 0x00, 0xFF }, { 0x03, 0x0C }, { 0x06, 0x24 },
        { 0x09, 0x00 }, { 0x0C, 0x24 }, { 0x0E, 0xE8 },
        { 0x13, 0xC2 }, { 0x15, 0x00 }
    }
};

// _IXACTSoundBank_PrepareEx@12
// call@0x17 -> XREF_XACT5849_SOUNDBANK_PREPARE
SOOVPA<9> IXACTSoundBank_PrepareEx_1_0_5849 =
{
    0, 9, -1, 1,
    {
        { 0x17, XREF_XACT5849_SOUNDBANK_PREPARE },
        { 0x00, 0x56 }, { 0x07, 0xFF }, { 0x0E, 0xFF },
        { 0x15, 0x14 }, { 0x1C, 0xF6 }, { 0x21, 0x68 },
        { 0x2C, 0x8B }, { 0x32, 0x00 }
    }
};

// ?Play@CSoundBank@XACT@@QAGJPBU_XACT_PREPARE_SOUNDCUE@@PAPAUIXACTSoundCue@@@Z
SOOVPA<8> XACTSoundBank_PlayInternal_1_0_5849 =
{
    0, 8, XREF_XACT5849_SOUNDBANK_PLAY, 0,
    {
        { 0x00, 0x55 }, { 0x22, 0xE8 }, { 0x48, 0x0C },
        { 0x6D, 0xF4 }, { 0x91, 0x08 }, { 0xB6, 0xFF },
        { 0xDC, 0xEB }, { 0xFF, 0x3B }
    }
};

// _IXACTSoundBank_PlayEx@12
// call@0x17 -> XREF_XACT5849_SOUNDBANK_PLAY
SOOVPA<9> IXACTSoundBank_PlayEx_1_0_5849 =
{
    0, 9, -1, 1,
    {
        { 0x17, XREF_XACT5849_SOUNDBANK_PLAY },
        { 0x00, 0x56 }, { 0x07, 0xFF }, { 0x0E, 0xFF },
        { 0x15, 0x14 }, { 0x1C, 0xF6 }, { 0x21, 0x68 },
        { 0x2C, 0x8B }, { 0x32, 0x00 }
    }
};

// _IXACTSoundBank_Stop@16
SOOVPA<8> IXACTSoundBank_Stop_1_0_5849 =
{
    0, 8, -1, 0,
    {
        { 0x00, 0x56 }, { 0x07, 0xFF }, { 0x0F, 0x74 },
        { 0x17, 0x74 }, { 0x1F, 0x85 }, { 0x25, 0x68 },
        { 0x30, 0x8B }, { 0x36, 0x00 }
    }
};

OOVPATable XACTENG_1_0_5849[] =
{
    {
        (OOVPA*)&XACTEngineDoWork_1_0_5849,
        XTL::EmuXACTEngineDoWork,
        #ifdef _DEBUG_TRACE
        "EmuXACTEngineDoWork"
        #endif
    },
    {
        (OOVPA*)&XACTEngineCreate_1_0_5849,
        XTL::EmuXACTEngineCreate,
        #ifdef _DEBUG_TRACE
        "EmuXACTEngineCreate"
        #endif
    },
    {
        (OOVPA*)&IXACTEngine_AddRef_1_0_5849,
        XTL::EmuIXACTEngine_AddRef,
        #ifdef _DEBUG_TRACE
        "EmuIXACTEngine_AddRef"
        #endif
    },
    {
        (OOVPA*)&XACTEngine_ReleaseInternal_1_0_5849, 0,
        #ifdef _DEBUG_TRACE
        "XACT::CEngine::Release (XREF)"
        #endif
    },
    {
        (OOVPA*)&IXACTEngine_Release_1_0_5849,
        XTL::EmuIXACTEngine_Release,
        #ifdef _DEBUG_TRACE
        "EmuIXACTEngine_Release"
        #endif
    },
    {
        (OOVPA*)&XACTEngine_RegisterWaveBankInternal_1_0_5849, 0,
        #ifdef _DEBUG_TRACE
        "XACT::CEngine::RegisterWaveBank (XREF)"
        #endif
    },
    {
        (OOVPA*)&IXACTEngine_RegisterWaveBank_1_0_5849,
        XTL::EmuIXACTEngine_RegisterWaveBank,
        #ifdef _DEBUG_TRACE
        "EmuIXACTEngine_RegisterWaveBank"
        #endif
    },
    {
        (OOVPA*)&IXACTEngine_UnRegisterWaveBank_1_0_5849,
        XTL::EmuIXACTEngine_UnRegisterWaveBank,
        #ifdef _DEBUG_TRACE
        "EmuIXACTEngine_UnRegisterWaveBank"
        #endif
    },
    {
        (OOVPA*)&XACTEngine_CreateSoundBankInternal_1_0_5849, 0,
        #ifdef _DEBUG_TRACE
        "XACT::CEngine::CreateSoundBank (XREF)"
        #endif
    },
    {
        (OOVPA*)&IXACTEngine_CreateSoundBank_1_0_5849,
        XTL::EmuIXACTEngine_CreateSoundBank,
        #ifdef _DEBUG_TRACE
        "EmuIXACTEngine_CreateSoundBank"
        #endif
    },
    {
        (OOVPA*)&IXACTSoundBank_GetSoundCueIndexFromFriendlyName_1_0_5849,
        XTL::EmuIXACTSoundBank_GetSoundCueIndexFromFriendlyName,
        #ifdef _DEBUG_TRACE
        "EmuIXACTSoundBank_GetSoundCueIndexFromFriendlyName"
        #endif
    },
    {
        (OOVPA*)&XACTSoundBank_AddRefInternal_1_0_5849, 0,
        #ifdef _DEBUG_TRACE
        "XACT::CSoundBank::AddRef (XREF)"
        #endif
    },
    {
        (OOVPA*)&IXACTSoundBank_AddRef_1_0_5849,
        XTL::EmuIXACTSoundBank_AddRef,
        #ifdef _DEBUG_TRACE
        "EmuIXACTSoundBank_AddRef"
        #endif
    },
    {
        (OOVPA*)&XACTSoundBank_DestructorInternal_1_0_5849, 0,
        #ifdef _DEBUG_TRACE
        "XACT::CSoundBank::~CSoundBank (XREF)"
        #endif
    },
    {
        (OOVPA*)&XACTSoundBank_DeletingDestructor_1_0_5849, 0,
        #ifdef _DEBUG_TRACE
        "XACT::CSoundBank deleting destructor (XREF)"
        #endif
    },
    {
        (OOVPA*)&XACTSoundBank_ReleaseInternal_1_0_5849, 0,
        #ifdef _DEBUG_TRACE
        "XACT::CSoundBank::Release (XREF)"
        #endif
    },
    {
        (OOVPA*)&IXACTSoundBank_Release_1_0_5849,
        XTL::EmuIXACTSoundBank_Release,
        #ifdef _DEBUG_TRACE
        "EmuIXACTSoundBank_Release"
        #endif
    },
    {
        (OOVPA*)&XACTSoundBank_PrepareInternal_1_0_5849, 0,
        #ifdef _DEBUG_TRACE
        "XACT::CSoundBank::Prepare (XREF)"
        #endif
    },
    {
        (OOVPA*)&IXACTSoundBank_PrepareEx_1_0_5849,
        XTL::EmuIXACTSoundBank_PrepareEx,
        #ifdef _DEBUG_TRACE
        "EmuIXACTSoundBank_PrepareEx"
        #endif
    },
    {
        (OOVPA*)&XACTSoundBank_PlayInternal_1_0_5849, 0,
        #ifdef _DEBUG_TRACE
        "XACT::CSoundBank::Play (XREF)"
        #endif
    },
    {
        (OOVPA*)&IXACTSoundBank_PlayEx_1_0_5849,
        XTL::EmuIXACTSoundBank_PlayEx,
        #ifdef _DEBUG_TRACE
        "EmuIXACTSoundBank_PlayEx"
        #endif
    },
    {
        (OOVPA*)&IXACTSoundBank_Stop_1_0_5849,
        XTL::EmuIXACTSoundBank_Stop,
        #ifdef _DEBUG_TRACE
        "EmuIXACTSoundBank_Stop"
        #endif
    }
};

uint32 XACTENG_1_0_5849_SIZE = sizeof(XACTENG_1_0_5849);
