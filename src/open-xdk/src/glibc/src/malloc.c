...
#if __STD_C
    extern Void_t* sbrk(ptrdiff_t i);
#else
    extern Void_t* sbrk();
#endif
#endif
...
#define largebin_index(sz)                                                                                                                                 \
    (((((unsigned long)(sz)) >> 6) <= 32) ? 56 + (((unsigned long)(sz)) >> 6) : ((((unsigned long)(sz)) >> 9) <= 20) ? 91 + (((unsigned long)(sz)) >> 9)   \
...
//
// *BIGBOY*  Have to switch global optimisations off around THIS function...
//
        ...
        */
        ...
        else
        {
            ...
            if(contiguous(av))
            {
                ...
                */
                ...
                {
                    volatile unsigned int Temp = ~pagemask;
                    correction += (((end_misalign + pagemask) & Temp) - end_misalign);
                }
        ...
        if((unsigned long)(size) >= (unsigned long)(nb + MINSIZE))
        {
...

            {
                ...
                //		sprintf(&s[0],"%x %x %x %x" , av, av->top,  p, pp);
                //		Print(40,140,&s[0]);
                //		Flip();
