...
void blit(Bitmap* dest, Bitmap* src, int dx, int dy, int sx, int sy, int sw, int sh, void (*blitter)(uint32*, uint32*, int, int), int parameter)
{
    ...
    if(xstarts >= src->w - width)
    {
        width -= (xstarts + width) - src->w;
    }
    if(ystarts >= src->h - height)
    {
        height -= (ystarts + height) - src->h;
    }
    ...
    if(xstartd >= dest->w - width)
    {
        width -= (xstartd + width) - dest->w;
    }
    if(ystartd >= dest->h - height)
    {
        height -= (ystartd + height) - dest->h;
    }
