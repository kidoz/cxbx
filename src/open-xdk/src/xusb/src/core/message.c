...
#include "core/hcd.h" /* for usbcore internals */
...
static int usb_start_wait_urb(struct urb* urb, int timeout, int* actual_length)
{
    ...
    if(!timeout && !awd.done)
    {
        ...
        {
            warn("usb_control/bulk_msg: timeout (ignore:%d)", 0);
...
int usb_set_interface(struct usb_device* dev, int interface, int alternate)
{
    ...
    for(i = 0; i < iface_as->desc.bNumEndpoints; i++)
    {
        ...
        (out ? dev->epmaxpacketout : dev->epmaxpacketin)[ep] = iface_as->endpoint[i].desc.wMaxPacketSize;
...
int usb_string(struct usb_device* dev, int index, char* buf, size_t size)
{
    ...
    if(!dev->have_langid)
    {
        err1 = usb_get_string(dev, 0, 0, tbuf, 4);
        if(err1 < 0)
        {
        ...
        {
            err("string descriptor 0 too short (ignore %d)", 0);
            err1 = -EINVAL;
            goto errout;
        }
        else
    ...
    err1 = usb_get_string(dev, dev->string_langid, index, tbuf, 2);
    if(err1 < 2)
        goto errout;
    len = tbuf[0];

    err1 = usb_get_string(dev, dev->string_langid, index, tbuf, len);
    if(err1 < 0)
        goto errout;

    size--; /* leave room for trailing NULL char in output buffer */
    for(idx = 0, u = 2; u < err1; u += 2)
    {
        if(idx >= size)
            break;
