...
static struct device_driver usb_generic_driver = { "usb", &usb_bus_type, generic_probe, generic_remove };
//	.name =	"usb",
//	.bus = &usb_bus_type,
//	.probe = generic_probe,
...
int usb_device_probe(struct device* dev)
{
    ...
    // ptr1 = dev
    // type1 = struct usb_interface
    // member = dev
    {
        struct device* __mptr = dev;
...
struct usb_interface* usb_ifnum_to_if(struct usb_device* dev, unsigned ifnum)
{
    ...
    for(i = 0; i < dev->actconfig->desc.bNumInterfaces; i++)
        if(dev->actconfig->interface[i].altsetting[0].desc.bInterfaceNumber == ifnum)
...
int usb_new_device(struct usb_device* dev, struct device* parent)
{
    ...
    for(i = 0; i < dev->actconfig->desc.bNumInterfaces; i++)
    {
        ...
        if(!desc->iInterface || usb_string(dev, desc->iInterface,
                                           interface->dev.name,
...
struct urb* usb_buffer_map(struct urb* urb)
{
    ...
    if(!urb || usb_pipecontrol(urb->pipe) || !urb->dev || !(bus = urb->dev->bus) || !(controller = bus->controller))
        return 0;
...
void usb_buffer_dmasync(struct urb* urb)
{
    ...
    if(!urb || !(urb->transfer_flags & URB_NO_DMA_MAP) || !urb->dev || !(bus = urb->dev->bus) || !(controller = bus->controller))
        return;
                      ...
                      struct scatterlist* sg, int nents)
{
    ...
    if(!dev || usb_pipecontrol(pipe) || !(bus = dev->bus) || !(controller = bus->controller) || !controller->dma_mask)
        return -1;
                           ...
                           struct scatterlist* sg, int n_hw_ents)
{
    ...
    if(!dev || !(bus = dev->bus) || !(controller = bus->controller) || !controller->dma_mask)
        return;
