...
static int rh_status_urb(struct usb_hcd* hcd, struct urb* urb)
{
    ...
    if(hcd->rh_timer.data || urb->status != -EINPROGRESS || urb->transfer_buffer_length < len)
    {
...
static void rh_report_status(unsigned long ptr)
{
    ...
    if(!urb->dev || urb->status != -EINPROGRESS || (hcd = urb->dev->bus->hcpriv) == 0 || !HCD_IS_RUNNING(hcd->state))
    {
...
static int hcd_submit_urb(struct urb* urb, int mem_flags)
{
     ...
     */
    if(!(urb->transfer_flags & URB_NO_DMA_MAP) && hcd->controller->dma_mask)
    {
        if(usb_pipecontrol(urb->pipe))
...
rescan:
    ...
    //
    //  list_entry(  (dev->urb_list)->next,   typeof(*urb),    urb_list  )
    ...
    /*        for (urb = list_entry((dev->urb_list)->next, urb, urb_list),1;
                 &urb->member != (dev->urb_list);
                 urb = list_entry(urb->member.next, urb, urb_list),1)
    */
    {
        int tmp = urb->pipe;
