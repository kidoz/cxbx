...
#ifdef DEBUG
static _inline char* portspeed(int portstatus)
{
...
static _inline struct device* hubdev(struct usb_device* dev)
{
                                    ...
                                    u16 portstatus, u16 portchange)
{
    ...
    for(i = 0; i < HUB_PROBE_TRIES; i++)
    {
        ...
        else if(dev->speed != USB_SPEED_HIGH && hub->speed == USB_SPEED_HIGH)
        {
            dev->tt = &hubstate->tt;
            dev->ttport = port + 1;
        }
