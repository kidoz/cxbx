...
static int usb_parse_interface(struct usb_interface* interface, unsigned char* buffer, int size)
{
    ...
    while(size > 0)
    {
        ...
        {
            struct usb_host_interface* usbptr;
            int oldmas;
            ...
            usbptr = kmalloc(sizeof(*usbptr) * interface->max_altsetting, GFP_KERNEL);
            if(usbptr == 0)
            { // NULL ) {
                err("couldn't kmalloc interface->altsetting");
                return -1;
            }
            memcpy(usbptr, interface->altsetting, sizeof(*interface->altsetting) * oldmas);
            kfree(interface->altsetting);
            interface->altsetting = usbptr;
        }
        ...
        memset(ifp->endpoint, 0, ifp->desc.bNumEndpoints * sizeof(struct usb_host_endpoint));
        ...
        if(size < USB_DT_INTERFACE_SIZE || d->bDescriptorType != USB_DT_INTERFACE || !d->bAlternateSetting)
            return parsed;
    }
...
int usb_get_configuration(struct usb_device* dev)
{
    ...
    memset(dev->config, 0, dev->descriptor.bNumConfigurations * sizeof(struct usb_host_config));
