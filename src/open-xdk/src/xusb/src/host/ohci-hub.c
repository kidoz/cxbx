...
static _inline u32 roothub_b(struct ohci_hcd* hc)
{
    return readl(&hc->regs->roothub.b);
}
static _inline u32 roothub_status(struct ohci_hcd* hc)
{
...
ohci_hub_status_data(struct usb_hcd* hcd, char* buf)
{
    ...
    for(i = 0; i < ports; i++)
    {
        ...
        status &= RH_PS_CSC | RH_PS_PESC | RH_PS_PSSC | RH_PS_OCIC | RH_PS_PRSC;
        if(status)
        {
            changed = 1;
            if(i < 7)
