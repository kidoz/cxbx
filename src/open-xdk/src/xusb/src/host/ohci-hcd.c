...
static _inline void disable(struct ohci_hcd* ohci)
{
    ohci->disabled = 1;
    ...
    int mem_flags)
{
    ...
    switch(ed->type)
    {
        ...
        default:
            ...
            if(size == 0)
                size++;
            else if((urb->transfer_flags & URB_ZERO_PACKET) != 0 && (urb->transfer_buffer_length % usb_maxpacket(urb->dev, pipe,
                                                                                                                 usb_pipeout(pipe))) == 0)
                size++;
            break;
...
static int hc_start(struct ohci_hcd* ohci)
{
     ...
     */
    if((readl(&ohci->regs->fminterval) & 0x3fff0000) == 0 || !readl(&ohci->regs->periodicstart))
    {
