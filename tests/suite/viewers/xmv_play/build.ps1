# Build xmv_play (minimal XMV video player) with the real Xbox XDK 5849
# toolchain (links xmv.lib) and stage the SDK's Test.xmv so the guest can open
# D:\Media\Videos\Test.xmv.
...
& "$vc\Link.Exe" /nologo /MACHINE:I386 /FIXED:NO /SUBSYSTEM:XBOX /INCREMENTAL:NO `
    ...
    /OUT:"$bin\xmv_play.exe" /MAP:"$bin\xmv_play.map" `
...
& (Join-Path $xdk "xbox\bin\imagebld.exe") /NOLIBWARN /DONTMOUNTUD `
    /STACK:0x10000 /TESTNAME:xmv_play /TESTID:0xFFFF0011 `
    /IN:"$bin\xmv_play.exe" /MAP:"$bin\xmv_play.map" /OUT:"$bin\default.xbe"
