# Build d3d_perf with the Xbox XDK 5849 performance-instrumented D3D library.
...
& "$vc\CL.Exe" /nologo /c /O2 /Gy /D_XBOX /DNDEBUG /DPROFILE /ML /W3 `
...
& "$vc\Link.Exe" /nologo /MACHINE:I386 /FIXED:NO /SUBSYSTEM:XBOX /INCREMENTAL:NO `
    ...
    /OUT:"$bin\d3d_perf.exe" /MAP:"$bin\d3d_perf.map" `
    "$bin\main.obj" xperf.lib xbdm.lib xapilib.lib d3d8i.lib xboxkrnl.lib
if ($LASTEXITCODE) { throw "Link failed ($LASTEXITCODE)" }

& (Join-Path $xdk "xbox\bin\imagebld.exe") /NOLIBWARN /DONTMOUNTUD `
    /STACK:0x10000 /TESTNAME:d3d_perf /TESTID:0xFFFF0012 `
    /IN:"$bin\d3d_perf.exe" /MAP:"$bin\d3d_perf.map" /OUT:"$bin\default.xbe"
