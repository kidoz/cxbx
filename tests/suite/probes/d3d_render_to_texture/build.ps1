# Build with the archived Xbox XDK 4627 toolchain so the probe exercises the
# exact retail CreateTexture2 / SetRenderTarget bodies used by Turok - Evolution.
...
& "$vc\Link.Exe" /nologo /MACHINE:I386 /FIXED:NO /SUBSYSTEM:XBOX /INCREMENTAL:NO `
    ...
    /OUT:"$bin\d3d_render_to_texture.exe" /MAP:"$bin\d3d_render_to_texture.map" `
...
& (Join-Path $xboxbin "imagebld.exe") /NOLIBWARN /DONTMOUNTUD `
    /STACK:0x10000 /TESTNAME:d3d_render_to_texture /TESTID:0xFFFF0014 `
    /IN:"$bin\d3d_render_to_texture.exe" /OUT:"$bin\default.xbe"
