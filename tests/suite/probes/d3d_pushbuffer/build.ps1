# Build d3d_pushbuffer with the real Xbox XDK 5849 retail libraries.
...
& "$vc\Link.Exe" /nologo /MACHINE:I386 /FIXED:NO /SUBSYSTEM:XBOX /INCREMENTAL:NO `
    ...
    /OUT:"$bin\d3d_pushbuffer.exe" /MAP:"$bin\d3d_pushbuffer.map" `
...
& (Join-Path $xdk "xbox\bin\imagebld.exe") /NOLIBWARN /DONTMOUNTUD `
    /STACK:0x10000 /TESTNAME:d3d_pushbuffer /TESTID:0xFFFF0013 `
    /IN:"$bin\d3d_pushbuffer.exe" /MAP:"$bin\d3d_pushbuffer.map" /OUT:"$bin\default.xbe"
