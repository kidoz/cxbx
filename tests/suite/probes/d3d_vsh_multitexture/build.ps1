...
& "$vc\Link.Exe" /nologo /MACHINE:I386 /FIXED:NO /SUBSYSTEM:XBOX /INCREMENTAL:NO `
    ...
    /OUT:"$bin\d3d_vsh_multitexture.exe" /MAP:"$bin\d3d_vsh_multitexture.map" `
...
& (Join-Path $xdk "xbox\bin\imagebld.exe") /NOLIBWARN /DONTMOUNTUD `
    /STACK:0x10000 /TESTNAME:d3d_vsh_multitexture /TESTID:0xFFFF0039 `
    /IN:"$bin\d3d_vsh_multitexture.exe" /MAP:"$bin\d3d_vsh_multitexture.map" `
    /OUT:"$bin\default.xbe"
