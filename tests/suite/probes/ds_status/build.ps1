# Build ds_status with the real Xbox XDK 5849 toolchain.
...
& "$vc\Link.Exe" /nologo /MACHINE:I386 /FIXED:NO /SUBSYSTEM:XBOX /INCREMENTAL:NO `
    ...
    /OUT:"$bin\ds_status.exe" /MAP:"$bin\ds_status.map" `
...
& (Join-Path $xdk "xbox\bin\imagebld.exe") /NOLIBWARN /DONTMOUNTUD `
    /STACK:0x10000 /TESTNAME:ds_status /TESTID:0xFFFF0009 `
    /IN:"$bin\ds_status.exe" /MAP:"$bin\ds_status.map" /OUT:"$bin\default.xbe"
