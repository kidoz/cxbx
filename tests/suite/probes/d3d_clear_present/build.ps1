# Build d3d_clear_present with the real Xbox XDK 5849 toolchain.
...
& "$vc\Link.Exe" /nologo /MACHINE:I386 /FIXED:NO /SUBSYSTEM:XBOX /INCREMENTAL:NO `
    ...
    /OUT:"$bin\d3d_clear_present.exe" /MAP:"$bin\d3d_clear_present.map" `
...
& (Join-Path $xdk "xbox\bin\imagebld.exe") /NOLIBWARN /DONTMOUNTUD `
    /STACK:0x10000 /TESTNAME:d3d_clear_present /TESTID:0xFFFF0003 `
    /IN:"$bin\d3d_clear_present.exe" /MAP:"$bin\d3d_clear_present.map" /OUT:"$bin\default.xbe"
