# Build the XACT 5849 in-memory wave-bank lifecycle probe.
...
& "$vc\Link.Exe" /nologo /MACHINE:I386 /FIXED:NO /SUBSYSTEM:XBOX /INCREMENTAL:NO `
    /LIBPATH:"$xdk\xbox\lib" /OUT:"$bin\xact_wavebank.exe" `
    /MAP:"$bin\xact_wavebank.map" "$bin\main.obj" `
...
& (Join-Path $xdk "xbox\bin\imagebld.exe") /NOLIBWARN /DONTMOUNTUD `
    /STACK:0x10000 /TESTNAME:xact_wavebank /TESTID:0xFFFF0044 `
    /IN:"$bin\xact_wavebank.exe" /MAP:"$bin\xact_wavebank.map" `
    /OUT:"$bin\default.xbe"
