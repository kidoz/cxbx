# Build d3d_pixel_shader with the real Xbox XDK 5849 toolchain.
...
& "$vc\Link.Exe" /nologo /MACHINE:I386 /FIXED:NO /SUBSYSTEM:XBOX /INCREMENTAL:NO `
    ...
    /OUT:"$bin\d3d_pixel_shader.exe" /MAP:"$bin\d3d_pixel_shader.map" `
...
& (Join-Path $xdk "xbox\bin\imagebld.exe") /NOLIBWARN /DONTMOUNTUD `
    /STACK:0x10000 /TESTNAME:d3d_pixel_shader /TESTID:0xFFFF000C `
    /IN:"$bin\d3d_pixel_shader.exe" /MAP:"$bin\d3d_pixel_shader.map" /OUT:"$bin\default.xbe"
