...
# exact retail D3D function body used by Turok - Evolution.
...
& "$vc\Link.Exe" /nologo /MACHINE:I386 /FIXED:NO /SUBSYSTEM:XBOX /INCREMENTAL:NO `
    ...
    /OUT:"$bin\d3d_shader_lifecycle.exe" /MAP:"$bin\d3d_shader_lifecycle.map" `
...
& (Join-Path $xboxbin "imagebld.exe") /NOLIBWARN /DONTMOUNTUD `
    /STACK:0x10000 /TESTNAME:d3d_shader_lifecycle /TESTID:0xFFFF0010 `
    /IN:"$bin\d3d_shader_lifecycle.exe" /OUT:"$bin\default.xbe"
