# Build d3d_depth with the real Xbox XDK 5849 toolchain.
...
& "$vc\Link.Exe" /nologo /MACHINE:I386 /FIXED:NO /SUBSYSTEM:XBOX /INCREMENTAL:NO `
    ...
    /OUT:"$bin\d3d_depth.exe" /MAP:"$bin\d3d_depth.map" `
...
& (Join-Path $xdk "xbox\bin\imagebld.exe") /NOLIBWARN /DONTMOUNTUD `
    /STACK:0x10000 /TESTNAME:d3d_depth /TESTID:0xFFFF0051 `
    /IN:"$bin\d3d_depth.exe" /MAP:"$bin\d3d_depth.map" /OUT:"$bin\default.xbe"
