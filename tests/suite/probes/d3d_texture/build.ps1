# Build d3d_texture with the real Xbox XDK 5849 toolchain.
...
& "$vc\Link.Exe" /nologo /MACHINE:I386 /FIXED:NO /SUBSYSTEM:XBOX /INCREMENTAL:NO `
    ...
    /OUT:"$bin\d3d_texture.exe" /MAP:"$bin\d3d_texture.map" `
...
& (Join-Path $xdk "xbox\bin\imagebld.exe") /NOLIBWARN /DONTMOUNTUD `
    /STACK:0x10000 /TESTNAME:d3d_texture /TESTID:0xFFFF0005 `
    /IN:"$bin\d3d_texture.exe" /MAP:"$bin\d3d_texture.map" /OUT:"$bin\default.xbe"
