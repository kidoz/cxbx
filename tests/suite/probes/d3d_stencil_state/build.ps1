...
# exact retail stencil-state function body used by Turok - Evolution.
...
& "$vc\Link.Exe" /nologo /MACHINE:I386 /FIXED:NO /SUBSYSTEM:XBOX /INCREMENTAL:NO `
    ...
    /OUT:"$bin\d3d_stencil_state.exe" /MAP:"$bin\d3d_stencil_state.map" `
...
& (Join-Path $xboxbin "imagebld.exe") /NOLIBWARN /DONTMOUNTUD `
    /STACK:0x10000 /TESTNAME:d3d_stencil_state /TESTID:0xFFFF0011 `
    /IN:"$bin\d3d_stencil_state.exe" /OUT:"$bin\default.xbe"
