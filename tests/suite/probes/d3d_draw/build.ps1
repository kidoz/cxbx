# Build d3d_draw with the real Xbox XDK 5849 toolchain.
...
& "$vc\Link.Exe" /nologo /MACHINE:I386 /FIXED:NO /SUBSYSTEM:XBOX /INCREMENTAL:NO `
    ...
    /OUT:"$bin\d3d_draw.exe" /MAP:"$bin\d3d_draw.map" `
...
& (Join-Path $xdk "xbox\bin\imagebld.exe") /NOLIBWARN /DONTMOUNTUD `
    /STACK:0x10000 /TESTNAME:d3d_draw /TESTID:0xFFFF0004 `
    /IN:"$bin\d3d_draw.exe" /MAP:"$bin\d3d_draw.map" /OUT:"$bin\default.xbe"
